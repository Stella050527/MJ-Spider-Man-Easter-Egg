...
- Enter 在 `window` 捕获阶段监听；GitHub 等由前端框架接管键盘事件的输入框也能识别。
- Enter 原本用于换行时，换行照常发生，彩蛋同时播放。
- 若网站未把 Enter 键事件继续传到页面，但确实提交了包含该编辑框的表单，扩展会在 `submit` 阶段严格复核内容并触发。
...
地址栏导航后的目标网页不会继承地址栏的用户激活状态。为避免 Chromium 把有声视频降级为静音，扩展使用 `offscreen` 文档播放本地音轨，并让页面中的透明视频静音同步播放；这只用于已经确认的地址栏 `mj` 搜索。
...
- 将 `effectAudioEnabled` 改为 `false` 可关闭所有彩蛋音效。普通网页触发直接播放视频音轨；地址栏触发由扩展的离屏文档播放同一份本地音轨。若普通网页仍被自动播放策略拒绝，动画会静音重试。
...
地址栏有声音的实现依赖 Chromium 109 引入的 Offscreen API，因此建议使用 Chrome 109、Edge 109 或更新版本。
...
- 地址栏触发的音轨通过 `AUDIO_PLAYBACK` 类型的 offscreen 文档播放；同一时间只保留一条离屏音轨，页面视频会静音以避免双重声音。
...
`scripting` 和 `<all_urls>` 权限用于在 Chrome/Edge 扩展重载后向已打开的普通网页补注入脚本，以及让触发能力覆盖用户访问的网站。`webNavigation` 仅用于识别由地址栏发起的已提交导航及其目标 URL。`offscreen` 仅用于在地址栏导航后的新页面播放扩展内置音轨。代码只处理触发事件和本地媒体播放，不保存浏览记录。
...
├─ src/                     # 扩展运行代码
│  ├─ config.js             # 快捷键与音效配置
│  ├─ offscreen.html        # 地址栏触发的离屏音频文档
│  ├─ offscreen-audio.js    # 离屏音轨播放器
│  └─ ...
├─ tests/                   # 规则与浏览器测试页面
├─ 蜘蛛侠-倒挂/             # 上游源素材
├─ 蜘蛛侠-耍帅/             # 上游源素材
